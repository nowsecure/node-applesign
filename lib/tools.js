  ...
  for (let key of keys) {
    which(key, function (err, loc) {
      if (err !== undefined) {
        cmd[key] = loc;
        if (--pending === 0) {
          cb(new Error('Warning: tools.findInPath: ' + err), user);
        }
      }
    });
  }
...
function getMobileProvisionPlist (file, cb) {
  const parseMobileProvisioning = (error, stdout) => {
    let res = null;
    if (!error) {
      if (stdout === '') {
        return cb(new Error(`Empty entitlements for ${file}.\nAre you providing a mobile provisioning with -m?`));
      }
      try {
          res = plist.parse(stdout);
      } catch (e) {
          error = e;
      }
    }
